...
# result_rules_file_path = 'subSET' + os_sep + time.strftime('%Y-%m-%d__%H:%M:%S', time.localtime(time.time())) + '.rules'a
source_PCAP_folder = 'PCAPsample' + os_sep + '(2)'
result_rules_file_path = 'PCAPsample' + os_sep + '(2)' + '__' + time.strftime('%Y-%m-%d__%H-%M', time.localtime(time.time())) + '.rules'
...
allow_txt_sample_count = 3  # 允许使用的txt样本个数（算法性能问题导致的，有待优化）
